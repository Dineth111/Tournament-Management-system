...
export default function OrganizerRegistrations() {
  ...
  const handleAiSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await sendChatMessage(aiQuestion);
      setAiResponse(response.reply || response.message || 'No response from AI assistant');
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          <button
            ...
            className="flex items-center px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg hover:from-indigo-700 hover:to-purple-700 transition duration-200 shadow-md"
        ...
        {aiAssistantOpen && (
          <div className="bg-white rounded-xl shadow-lg p-6 mb-8 transition-all duration-300">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold text-gray-800">AI Tournament Assistant</h3>
              <button 
            ...
            <p className="text-gray-600 mb-4">Ask me anything about tournament registrations, player statistics, or event planning.</p>
            ...
            <form onSubmit={handleAiSubmit} className="mb-4">
              <div className="flex">
                <input
                  ...
                  placeholder="Ask about registrations, player stats, or tournament planning..."
                  ...
                  disabled={aiLoading}
                  aria-label="Ask a question to the AI assistant"
                />
                <button
                  type="submit"
                  disabled={aiLoading}
                  className="px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-r-lg hover:from-indigo-700 hover:to-purple-700 transition duration-200 disabled:opacity-50"
            ...
            {/* AI Suggestions */}
            <div className="mt-4">
              <h4 className="font-semibold text-gray-700 mb-2">Try asking:</h4>
              <div className="flex flex-wrap gap-2">
                <button 
                  onClick={() => setAiQuestion("How many players are registered for the National Championship?")}
                  className="px-3 py-1 bg-indigo-100 text-indigo-700 rounded-full text-sm hover:bg-indigo-200 transition duration-200"
                  aria-label="Ask about player count for National Championship"
                >
                  Player count for National Championship
                </button>
                <button 
                  onClick={() => setAiQuestion("What categories have the most registrations?")}
                  className="px-3 py-1 bg-indigo-100 text-indigo-700 rounded-full text-sm hover:bg-indigo-200 transition duration-200"
                  aria-label="Ask about popular categories"
                >
                  Popular categories
                </button>
                <button 
                  onClick={() => setAiQuestion("Show me pending registrations")}
                  className="px-3 py-1 bg-indigo-100 text-indigo-700 rounded-full text-sm hover:bg-indigo-200 transition duration-200"
                  aria-label="Ask about pending registrations"
                >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex space-x-2">
              <button
                ...
                aria-label="Show all registrations"
              >
                All
              </button>
              <button
                ...
                aria-label="Show confirmed registrations"
              >
                Confirmed
              </button>
              <button
                ...
                aria-label="Show pending registrations"
              >
                Pending
              </button>
            </div>
