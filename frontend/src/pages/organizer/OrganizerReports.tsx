...
export default function OrganizerReports() {
  ...
  const generateAiInsights = async () => {
    ...
    try {
      const prompt = `Based on the following tournament data, provide insights and recommendations:
      
      Tournament Reports:
      ${tournamentReports.map(t => `- ${t.name}: ${t.participants} participants, $${t.revenue} revenue, ${t.status}`).join('\n')}
      
      Performance Reports:
      ${performanceReports.map(p => `- ${p.category}: ${p.completionRate}% completion rate, ${p.pending} pending matches`).join('\n')}
      ...
      2. Recommendations for improving future tournaments
      3. Areas that need attention
      `;
      
      const response = await sendChatMessage(prompt);
      setAiInsights(response.reply || response.message || 'No insights generated');
    } catch (error) {
        ...
        {aiInsightsOpen && (
          <div className="bg-white rounded-xl shadow-lg p-6 mb-8 transition-all duration-300">
            ...
            {aiLoading ? (
              ...
              <p className="text-gray-600 text-center py-4">No insights available</p>
            )}
            ...
            <div className="mt-4 flex justify-center">
              <button
                ...
                disabled={aiLoading}
                className="flex items-center px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg hover:from-indigo-700 hover:to-purple-700 transition duration-200 disabled:opacity-50"
          ...
          <div className="border-b border-gray-200">
            <nav className="flex">
              <button
                ...
                aria-label="View tournament reports"
              >
                Tournament Reports
              </button>
              <button
              ...
              <div>
                <div className="flex justify-between items-center mb-6">
                  ...
                  <div className="flex space-x-3">
                    ...
                    <button 
                      onClick={() => exportTournamentReportsToPDF(tournamentReports)}
                      ...
                      aria-label="Export tournament reports to PDF"
                    >
                      <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {activeTab === 'performance' && (
              <div>
                <div className="flex justify-between items-center mb-6">
                  ...
                  <button 
                    onClick={() => exportPerformanceReportsToPDF(performanceReports)}
                    className="px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg hover:from-indigo-700 hover:to-purple-700 transition duration-200 flex items-center"
                    aria-label="Export performance reports to PDF"
                  >
                    <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
            ...
            {activeTab === 'financial' && (
              <div>
                <div className="flex justify-between items-center mb-6">
                  ...
                  <button 
                    onClick={() => exportFinancialReportsToPDF(tournamentReports)}
                    className="px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg hover:from-indigo-700 hover:to-purple-700 transition duration-200 flex items-center"
                    aria-label="Export financial reports to PDF"
                  >
                    <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
