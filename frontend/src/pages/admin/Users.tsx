...
export default function Users() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const usersWithDefaults = response.data.map((user: any) => ({
        ...user,
    ...
    try {
      if (editingUser) {
        ...
        setUsers(users.map(user => 
          user._id === editingUser._id ? {...response.data, createdAt: response.data.createdAt || editingUser.createdAt} : user
        ));
      } else {
        // Create new user
        const response = await adminApi.createUser(formData);
        toast.success('User created successfully');
        setUsers([...users, {...response.data, createdAt: response.data.createdAt || new Date().toISOString()}]);
      }
