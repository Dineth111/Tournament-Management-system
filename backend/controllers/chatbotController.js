    ...
    if (provider === 'openai') {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 || response.status === 403) {
          const errMsg = 'Invalid API key. Please contact the administrator to configure a valid API key.';
          console.error('OpenAI API key error:', errJson || errText);
      ...
      if (!response.ok) {
        ...
        // Handle API key errors specifically for Google
        if (response.status === 400 || response.status === 403) {
          // Check if the error message indicates a leaked API key
          const errorDetails = errJson?.error || {};
          const errorMessage = errorDetails.message || '';
          
          if (errorMessage.includes('API key not valid') || 
              errorMessage.includes('API key was reported') ||
              errorMessage.includes('leaked') ||
              errorMessage.includes('compromised')) {
            const errMsg = 'The API key was reported as leaked or compromised. Please contact the administrator to configure a new API key.';
            console.error('Google Gemini API key error:', errJson || errText);
            return res.status(response.status).json({ 
              success: false, 
              message: errMsg, 
              error: errJson || errText,
